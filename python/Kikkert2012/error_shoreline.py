...
sauvegarde = True
titre_sauv = "error_shoreline_numCorr"
...
symb_legend = [
    ...
    Line2D([0], [0], label=nom_plots[3], marker="+", color="red", linestyle=""),
]
    ...
    for i in range(len(val_expBin) - 1):
        ...
        if intersec is not None:
            ...
            if tExp[i_exp] > 6:
                ...
                # Retrieve exp points at ± 0.5 m from the intersection found in the binned data
...
for ind_path, path in enumerate(paths):
    ...
    list_intersec = np.zeros((len(temps), 2))
    ...
    for ind_t, time in enumerate(temps_str):
        ...
        if len(taille_contours) > 0:
            pos_biggest_contours = np.argmax(
                taille_contours
            )  # récupération de la position du plus grand contour
        ...
        if float(time) > 6.5:
            ...
            for i in range(len(val_numBin) - 1):
                ...
                if intersec is not None:
                    ...
                    list_intersec[ind_t] = intersec
                    break

            if inter == 0:
                list_intersec[ind_t] = np.array([(xNumBin[i] + xNumBin[i + 1]) / 2, 0])
            ...
            list_intersec[ind_t] = all_intersec[np.argmin(all_intersec[:, 0])]

    # interpolation on experimental time steps
    list_intersec_interp = np.interp(tExp, temps, list_intersec[:, 0])

    # calculate skill scores
    RMSE[ind_path] = np.sqrt(
        sum((list_intersec_interp - list_intersec_res) ** 2) / len(list_intersec_interp)
    )
    BSS[ind_path] = 1 - sum((list_intersec_res - list_intersec_interp) ** 2) / sum(
        (list_intersec_res - np.mean(list_intersec_res)) ** 2
    )
    WSS[ind_path] = 1 - sum((list_intersec_res - list_intersec_interp) ** 2) / sum(
        (
            abs(list_intersec_res - np.mean(list_intersec_res))
            + abs(list_intersec_interp - np.mean(list_intersec_res))
        )
        ** 2
    )


fig = plt.figure(num=3, figsize=(8, 16), dpi=300)
...
for i in range(len(RMSE)):
    ...
    ax1.scatter(
        ...
        color=clmap[i],
        marker="o",
        label=nom_plots[i] + " : RMSE = " + str(RMSE[i]),
    )
    ...
    ax2.scatter(
        ...
        color=clmap[i],
        marker="o",
        label=nom_plots[i] + " : BSS = " + str(BSS[i]),
    )
    ...
    ax3.scatter(
        ...
        color=clmap[i],
        marker="o",
        label=nom_plots[i] + " : WSS = " + str(WSS[i]),
    )
